...
def _load_window(
    db: Session, holding: Holding, start: date, today: date
) -> tuple[list[PriceHistory], bool, str | None]:
    cached = _cached(db, holding, start, today)
    fetched_today = bool(
        cached and cached[-1].fetched_at and cached[-1].fetched_at.date() == today
    )
    warning = None
    stale = False
    missing_ranges = []
        ...
        try:
            ...
            for fetch_start, fetch_end in missing_ranges:
                rows = fetch_price_history(
                    holding.code, holding.type, fetch_start, fetch_end
                )
...
def holding_history(db: Session, holding: Holding, range_name: str) -> dict:
    ...
    cached, stale, warning = _load_window(db, holding, start, today)
...
def analysis_history(db: Session, holding: Holding) -> dict:
    today = local_now().date()
    start = today - timedelta(days=90)
    cached, stale, warning = _load_window(db, holding, start, today)
    if not cached:
        raise HistoryUnavailable("近期历史行情暂时不可用")
    selected = cached[-60:]
    if selected[-1].trade_date < today - timedelta(days=7):
        stale = True
        warning = warning or "最近交易日距离当前日期较远"
    return {
        "points": [
            {"trade_date": row.trade_date, "price": to_decimal(row.price)}
            for row in selected
        ],
        "sample_count": len(selected),
        "source": selected[-1].source,
        "last_trade_date": selected[-1].trade_date,
        "stale": stale,
        "warning": warning,
    }
