...
test('legacy 风险试算、刷新触发、告警查看与手动平仓', async ({ page }, testInfo) => {
  ...
  const prefillEntry = page.getByRole('button', { name: '填入新增持仓', exact: true })
  await expect(prefillEntry).toBeVisible()
  await prefillEntry.click()
  await expect(page).toHaveURL(/\/holdings\?create=1/)
  const prefillDialog = page.getByRole('dialog')
  await expect(prefillDialog.getByPlaceholder('例如 000001')).toHaveValue('000002')
  await expect(prefillDialog.getByPlaceholder('例如 平安银行')).toHaveValue('新仓试算标的')
  const prefillNumbers = prefillDialog.locator('.el-input-number input')
  await expect(prefillNumbers.nth(0)).toHaveValue(/^20(\.00)?$/)
  await expect(prefillNumbers.nth(1)).toHaveValue('400')
  await prefillDialog.getByRole('button', { name: '取消' }).click()
  await expect(prefillDialog).toBeHidden()
  ...
  await expect(page.getByRole('button', { name: '填入新增持仓', exact: true })).toHaveCount(0)
  await expect(page.locator('body')).not.toContainText(/推荐标的|自动下单|确认创建仓位|记录加仓/)
