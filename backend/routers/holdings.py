...
def _commit_legacy(db: Session) -> None:
    db.commit()
...
def create_holding(data: HoldingCreate, db: Session = Depends(get_db)):
    ...
    holding = Holding(
        ...
        current_price=to_decimal(0), highest_price=to_decimal(data.buy_price),
...
def get_holding(holding_id: int, db: Session = Depends(get_db)):
    if authority(db).stage == "new-authoritative":
        return position_holding_payload(db, _position_or_404(db, holding_id))
    return holding_payload(_get_holding(db, holding_id))


def _position_or_404(db: Session, position_id: int) -> Position:
