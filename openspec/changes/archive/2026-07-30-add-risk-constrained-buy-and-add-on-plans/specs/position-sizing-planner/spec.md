...
前端 SHALL 仅在运行时同时声明 `risk_plan_previews` 和 `risk_covered_position_creation` 时，允许有效新仓试算预填受风险约束的 Position 创建流程。只有 `risk_plan_previews` 可用的稳定 legacy/shadow-read 运行面 SHALL 保持试算只读，不显示 Position 创建、Holding 修改或自动记录加仓操作。
...
#### Scenario: 稳定运行面完成试算
- **WHEN** legacy 或 shadow-read 用户取得有效新仓或加仓试算结果
- **THEN** 页面只展示计算结果和重新试算操作，不调用 Holding 或 Position 写入 API
