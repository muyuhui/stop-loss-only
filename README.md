...
- 仪表盘"本月账本"卡片：按月统计已实现盈亏、平仓笔数、触发次数与止损调整次数（含调低次数），并标注当前最大浮亏来源；已实现与浮亏分开计算，不合并展示。
...
| 浏览器系统通知 | 支持（站内呈现，仅本机） |
| 桌面通知（Windows 本地） | 支持（后端直发，仅本机；完整/脱敏模式与演示暂停） |
...
**桌面通知（Windows 本地，仅本机呈现）**：新触发止损时，后端在告警提交后直接发送一条 Windows 本地通知（每次监控周期最多一条汇总），不依赖浏览器标签页是否打开；调度刷新与手动刷新共用同一钩子。设置页可控制：启用开关（默认关闭）、内容模式与演示模式。

- **完整模式**：通知包含持仓名称、代码与价格（多笔时列出前 3 笔并附总数）。
- **脱敏模式**：通知只显示"有 N 笔触发待处理"——不出现任何持仓名称、代码、价格，也不出现"止损"字样，Windows 通知中心历史同样安全；适合演示或共享屏幕场景。
- **演示模式**：完全暂停所有桌面通知。
- 通知发送失败只记录日志，绝不影响监控事实与周期结果；通知设置由后端持久化（`/api/settings`），浏览器通知（localStorage 偏好）与桌面通知（后端设置）互不干扰。
